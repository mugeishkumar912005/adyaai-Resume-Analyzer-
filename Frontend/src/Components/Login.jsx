...
const Login = () => {
    ...
    const ForgetPasswordHandler = async () => {
        if (email === "") {
            toast.warn("Please enter your email before clicking!");
            return;
        }

        try {
            const Mail_Gen = await axios.post("https://adyaai-resume-analyzer-back.onrender.com/api/user/Trigger", {
                Email: email
            });
            if (Mail_Gen.status === 200) {
                toast.success(Mail_Gen.data.Msg || "Mail sent successfully!");
            } else {
                toast.error(Mail_Gen.data.Msg || "Something went wrong.");
            }
        } catch (err) {
    ...
    const HandleLogin = async (e) => {
        ...
        try {
            const response = await axios.post('https://adyaai-resume-analyzer-back.onrender.com/api/user/Login', {
                Email: email,
                Password: password
            });
    ...
    return (
        <>
            ...
            <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4 relative overflow-hidden">
                ...
                <div className="max-w-md w-100 bg-white p-5 mt-10 rounded-lg shadow-md z-10 relative">
                    ...
                    <form className="space-y-5" onSubmit={HandleLogin}>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="text-sm">
                                <a href="#" className="font-medium text-blue-600 hover:text-blue-500" onClick={ForgetPasswordHandler}>
